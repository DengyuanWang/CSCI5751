...
if __name__=="__main__":
    ...
    bike_usage = get_used_time(end_year,end_month,end_month)
    max_usage = max(bike_usage.values())+1
    min_usage = min(bike_usage.values())
    interval = (max_usage-min_usage)/10.0
    usage_count = np.zeros(10)
    usage_distribution = np.arrange(min_usage, max_usage, interval)
    for bikeNumber in bike_usage:
        index = (int)((bike_usage.get(bikeNumber)-min_usage)/interval)
        np.put(usage_count,index,usage_count[index]+1)
    plt.bar(usage_distribution, usage_count)
    plt.show()
    #------------------------------population dense analysis---------------------------
    stationPopDense = get_pop_density(2011,1,12)
    topTenStation = sorted(stationPopDense.items(), key = lambda kv:(kv[1], kv[0]))
    count = 0
    for station in topTenStation:
        if count == 10:
            break
        print(station)
        count += 1
